...
from .serializers import ProductSerializer, IdsListSerializer, SellerProductSerializer, NetPriceSerializer, ProductSellerSerializer,\
...
class ProductDetail(APIView):
    ...
    def get(self, request, id, format=None, *args):
        try:
            response_data = self.product_view.get(int(id))
        except ObjectDoesNotExist:
...
class ProductSellerMapper(APIView):
    serializer_class = ProductSellerSerializer

    def post(self,request,pk):
        request.data._mutable=True
        request.data['seller_id']=pk
    ...
    def put(self, request, pk):
        request.data['seller_id'] = pk
        try:
            object = SellerProduct.objects.get(id=request.data['id'])
        except:
            return Response({"message":"Detail not found"}, status=status.HTTP_404_NOT_FOUND)
        request.data._mutable=True
        request.data['seller_id']=pk
        request.data._mutable=False
...
class ProductSellerStatus(generics.ListAPIView):
    ...
    def get(self,request,seller_id,status):
        seller_products=SellerProduct.objects.filter(seller_id=seller_id)
        ids=list(map(lambda x: x.product_id,filter(lambda x: Product.objects.get(id=x.product_id).status==status,seller_products)))
        product_view=ProductView()
        return Response(product_view.get_by_ids(ids))
